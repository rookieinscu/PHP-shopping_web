<?php
			$book_number=$_SESSION['A'];
            $db_connection = new mysqli(DB_HOST, DB_USER, DB_PASS, DB_NAME);
            if (!$db_connection->set_charset("utf8")) {
                $errors[] = $db_connection->error;
            }
            if (!$db_connection->connect_errno) {
                $sql = "SELECT book_owner, book_name, book_writer, book_edition, book_publisher, book_course, book_price, book_picture, book_introduction
                        FROM book_info
				...
				if ($result_of_login_check) {
                    $result_row = $result_of_login_check->fetch_object();
						...
						$_SESSION['book_owner'] = $result_row->book_owner;
						...
						$_SESSION['book_introduction'] = $result_row->book_introduction;
				}
		}
?>
